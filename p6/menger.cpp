...
"                    If omitted, a random mode will be chosen.\n"
...
bool initGL()
{
	...
	// float l_amb[] = {0.3,0.3,0.3,1};
	float l_amb[] = {0.5,0.5,0.5,1};
...
void drawFaces()
{
	/* NOTE with GL_QUADS, you have to be careful with the order
...
int main(int argc, char *argv[])
{
	...
	bool use_rand = true;
	srand(time(0));
	while ((c = getopt_long(argc, argv, "hFd:m:", long_opts, &opt_index)) != -1) {
		switch (c) {
			case 'h':
				printf(usage,argv[0]);
				return 0;
